...
export default function SellTracker({
  ...
  const handleCreateOrder = () => {
    ...
    setEditingOrder({
      ...
      discountType: 'fixed',
      paymentMethod: 'QR',
      agentId: settings.linkedAgent?.agentId,
      commissionValue: settings.linkedAgent?.defaultCommission,
  ...
  const [batchExportNote, setBatchExportNote] = useState('');
  ...
  const confirmExportBatch = async () => {
     ...
     const agentName = settings?.linkedAgent?.name || 'Agent';
     const note = batchExportNote.trim() || 'Export';
     const fileName = `${agentName} - ${note}.fgs`;
  ...
  const handleCreateEntry = () => {
    const firstName = settings.linkedAgent?.name?.split(' ')[0] || 'Agent';
                   ...
                   <div className="flex flex-col gap-4">
                      <div className="flex items-center gap-4 bg-app-card p-4 rounded-xl border border-app-border">
                         ...
                         <div className="ml-auto flex items-center gap-2">
                            <button
                               onClick={() => handleExportBatch(activeBatchId)}
                               className="flex items-center gap-2 px-4 py-2 bg-app-bg text-app-text rounded-xl border border-app-border hover:bg-app-card transition-all font-bold shadow-sm"
                            >
                               <Download size={18} /> <span className="hidden sm:inline">Export Entry</span>
                            </button>
                            <button
                  ...
                  <div key={batch.id} className="bg-app-card rounded-2xl border border-app-border overflow-hidden hover:shadow-md transition-all p-6 cursor-pointer group" onClick={() => setActiveBatchId(batch.id)}>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => { e.stopPropagation(); handleExportBatch(batch.id); }} className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-3 py-1.5 rounded-lg hover:bg-emerald-500/20 transition-all">
                ...
                {saleOrders.filter(o => !o.batchId).length > 0 && (
                  <div className="bg-app-card rounded-2xl border border-app-border overflow-hidden hover:shadow-md transition-all p-6 cursor-pointer group" onClick={() => setActiveBatchId('legacy')}>
                     <div className="flex justify-between items-start mb-4">
                       ...
                       <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                         <button onClick={(e) => { e.stopPropagation(); handleExportBatch('legacy'); }} className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-3 py-1.5 rounded-lg hover:bg-emerald-500/20 transition-all">
                            <Download size={14} /> Export
                         </button>
      ...
      {batchExportModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-app-card rounded-2xl shadow-2xl max-w-sm w-full p-8 border border-app-border space-y-6">
            ...
            <div className="space-y-4">
               <div>
                  <label className="block text-[10px] font-black text-app-muted uppercase mb-1 tracking-widest">Custom Note / Name</label>
                  <p className="text-xs text-app-muted mb-2">The system will automatically add your identity to the exported file name.</p>
                  <input
                     type="text"
                     placeholder="e.g., Monday Shift"
                     ...
                     value={batchExportNote}
                     onChange={(e) => setBatchExportNote(e.target.value)}
                  />
                  <p className="text-[10px] font-mono text-emerald-500/70 mt-2 truncate">Result: {settings?.linkedAgent?.name ? settings.linkedAgent.name.split(' ')[0] : 'Agent'} - {batchExportNote || 'Export'}.fgs</p>
               </div>
               
               <button
                  onClick={confirmExportBatch}
                  disabled={!batchExportNote.trim()}
                  className="w-full flex items-center justify-center gap-2 py-4 bg-emerald-500 text-white rounded-2xl hover:bg-emerald-600 transition-all font-black text-lg shadow-xl shadow-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
               >
